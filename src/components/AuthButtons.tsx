...
interface AuthButtonsProps {
  onAuthSuccess?: () => void;
  // When true, render buttons explicitly in order: Login, Register, Google
  inModalLayout?: boolean;
}

export const AuthButtons = ({ onAuthSuccess, inModalLayout }: AuthButtonsProps = {}) => {
    ...
    <div className="flex items-center gap-2">
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button size="sm" onClick={() => { setMode('login'); setOpen(true); }} className="flex items-center gap-2">
        ...
        <DialogContent className="sm:max-w-md w-[90vw] mx-auto">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            <DialogFooter className="flex flex-col gap-2">
              ...
              {/* In modal layout, keep footer minimal; Google button is rendered inline outside */}
            </DialogFooter>
          </form>
          {!inModalLayout && (
      ...
      {/* Inline order: Login, Register, Google */}
      {/* For non-modal usage, the DialogTrigger above is the Login button */}
      <Button size="sm" onClick={() => { setMode('register'); setOpen(true); }} className="flex items-center gap-2">
        <UserPlus className="h-4 w-4" />
        <span className="hidden sm:inline">Register</span>
      </Button>
      {inModalLayout && GoogleButtonInline}
    </div>
  );
};
